...
/**
 * Writes the icon stylesheet: base `.{baseClass}` plus `.{baseClass}.{classId}::before` rules for each glyph.
 *
 ...
 *
 * Defaults match the Vite plugin: `outputDir` → `./node_modules/.cache/zynora` (from `process.cwd()`), `fontName` → `zynora`,
 * `baseClass` → `zy`, `fontsUrl` inferred only when `outputDir` lies under a `public` path segment.
 *
 ...
 * @param {string} [options.outputDir] - Output directory (default: `node_modules/.cache/zynora` relative to `process.cwd()`).
...
export async function buildIconFont(options = {}) {
    ...
    const inputDir = options.inputDir !== undefined && options.inputDir !== null && String(options.inputDir).trim() !== ""
        ...
        : undefined;
    const outputDir = path.resolve(options.outputDir ?? resolveDefaultZynoraOutputDir(cwd));
