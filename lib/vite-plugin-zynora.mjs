 ...
 * @property {string} [output] - Generated kit folder; default `public/zynora` (relative to Vite root). `vite build` resolves `zynora/css` to that folder’s generated `.css` on disk so `url(./…)` resolves next to the real file (no virtual CSS in production).
    ...
    return {
        ...
        async load(id) {
            ...
            try {
                ...
                const inferredInPublic = inferFontsUrlFromPublicDir(
                    resolvedAbsolutePublicDir,
                    resolvedOutput ?? ""
                ) !== null;
                const shouldUseBundleSubdir = !inferredInPublic
                    && (resolvedFontsUrl === null || resolvedFontsUrl === undefined);
                ...
                const rewriteCtx = {
                    ...
                    ...(shouldUseBundleSubdir ? { bundleAsPublicSubdir: BUNDLE_FONT_SUBDIR } : {})
                };
