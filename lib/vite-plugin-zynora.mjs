...
import { resolveDefaultZynoraOutputDir, ZYNORA_PACKAGE_ICONS_DIR } from "./paths.mjs";
...
/**
 * When the kit is outside `publicDir` but `vite build` runs, fonts are emitted as Rollup assets under this
 * path (POSIX) so `url(/zynora/…)` in CSS matches files in `dist/`.
 */
const BUNDLE_FONT_SUBDIR = "zynora";
 ...
 * @param {object} [opts]
 * @param {string} [opts.bundleAsPublicSubdir] - When set (production build, kit outside `publicDir`), URL under this subdir in `dist/`.
 * @returns {string}
 */
function fontUrlForZynoraAsset(viteBase, publicDir, resolvedOutput, basename, opts = {}) {
    if (opts.bundleAsPublicSubdir) {
        const posix = `${opts.bundleAsPublicSubdir}/${basename}`.replace(/\/{2,}/g, "/");

        return urlForPublicFile(viteBase, posix);
    }
 ...
 * @param {string} [ctx.bundleAsPublicSubdir] - Passed through to {@link fontUrlForZynoraAsset}.
 * @returns {string}
 */
function rewriteZynoraVirtualCssFontUrls(css, ctx) {
    const { viteBase, publicDir, resolvedOutput, fontName, bundleAsPublicSubdir } = ctx;
    const urlOpts = bundleAsPublicSubdir ? { bundleAsPublicSubdir } : {};
    let out = css;

    for (const ext of ["woff2", "woff", "ttf"]) {
        const file = `${fontName}.${ext}`;
        const resolvedUrl = fontUrlForZynoraAsset(viteBase, publicDir, resolvedOutput, file, urlOpts);
...
function rewriteZynoraVirtualCssFontSrc(css, ctx) {
    ...
    const urlOpts = bundleAsPublicSubdir ? { bundleAsPublicSubdir } : {};
    const parts = [
        `url("${fontUrlForZynoraAsset(viteBase, publicDir, resolvedOutput, `${fontName}.woff2`, urlOpts)}") format("woff2")`,
        `url("${fontUrlForZynoraAsset(viteBase, publicDir, resolvedOutput, `${fontName}.woff`, urlOpts)}") format("woff")`,
        `url("${fontUrlForZynoraAsset(viteBase, publicDir, resolvedOutput, `${fontName}.ttf`, urlOpts)}") format("truetype")`
    ];
 ...
 * @property {string} [output] - Generated kit folder; default `node_modules/.cache/zynora` (relative to Vite root). Dev serves CSS/fonts via virtual module and `/@zynora/*`; production build still resolves `zynora/css` to the on-disk `.css` next to font files.
 * @property {string | null} [fontsUrl] - `@font-face` URL prefix in the **generated file**; omit for relative `./` (recommended for Vite). The dev virtual stylesheet still rewrites to `/zynora/…` or `/@zynora/…` as needed.
 ...
 *
 * In dev, a virtual CSS module rewrites `@font-face` URLs (and can serve `/@zynora/*` when output is outside `publicDir`).
 * In `vite build`, imports resolve to the generated stylesheet on disk so bundlers resolve `url(./font.woff2)` normally.
 */
...
export function zynora(userOptions = {}) {
    ...
    async function runBuild() {
        ...
        // Omitting user `fontsUrl`: write `./` in the generated CSS so `vite build` can resolve font files
        // next to the real stylesheet; the virtual module in dev still rewrites URLs for publicDir / `/@zynora/*`.
        await buildIconFont({
            packageIcons,
            ...
            fontsUrl: userOptions.fontsUrl !== undefined
                ? (userOptions.fontsUrl === "" ? null : userOptions.fontsUrl)
                : null,
    ...
    return {
        ...
        configResolved(config) {
            ...
            resolvedOutput = userOptions.output
                ...
                : resolveDefaultZynoraOutputDir(root);
        ...
        /**
         * Maps `zynora/css` to a virtual module in dev; in `vite build` resolves to the generated `.css` on disk
         * so `url(./zynora.woff2)` resolves next to that file (no virtual CSS, no unresolved `/zynora/...` in the bundle).
         *
         * @param {string} id - Requested module id.
         * @returns {string | undefined}
         */
        resolveId(id) {
            const wantsZynoraCss = id === VIRTUAL_CSS_ID
                || id === "zynora/css"
                || id.endsWith("/zynora/css")
                || isCssPackageEntry(id);

            if (wantsZynoraCss && viteResolvedConfig?.command === "build") {
                return getGeneratedCssPath();
            }
        ...
        async load(id) {
            ...
            try {
                ...
                // Dev virtual CSS must use real `publicDir` URLs or `/@zynora/*` (middleware), never
                // `bundleAsPublicSubdir` - that prefix is only for emitted files in `dist/` on build.
                const rewriteCtx = {
                    viteBase,
